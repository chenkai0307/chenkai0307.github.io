...
![紫罗兰永恒花园写信场景 PC 壁纸](https://bizhi1.com/wp-content/uploads/2024/11/violet-evergarden-writing-letter-christmas-scene-pc-wallpaper-4k.jpg)
...
![image-20220916213529426](https://oss.itbaima.cn/internal/markdown/2022/09/16/5mpBD1JyjCMGgnO.png)

只要源代码中存在报错的地方，就无法正常完成编译得到二进制文件，会提示构建失败：

![image-20220916213641899](https://oss.itbaima.cn/internal/markdown/2022/09/16/x5PjR9OAGMCQtS6.png)
...
![image-20220916214557378](https://oss.itbaima.cn/internal/markdown/2022/09/16/GLZdxf6B3Agu98N.png)

如果两段代码没有加上分号分割，那么编译器会认为这两段代码是同一句代码中的，即使出现换行或者是空格：

![image-20220916214736541](https://oss.itbaima.cn/internal/markdown/2022/09/16/ErQnpo2DVw7mJks.png)

这里IDEA很聪明，会提醒我们这里少加了分号，所以说这个IDEA能够在初期尽可能地帮助新手。

再比如下面的代码：

![image-20220916214822072](https://oss.itbaima.cn/internal/markdown/2022/09/16/sDcuan8MJ92l3P1.png)

![image-20220916214929651](https://oss.itbaima.cn/internal/markdown/2022/09/16/i1VFk6RUtp8XfMr.png)
...
![image-20220916221833145](https://oss.itbaima.cn/internal/markdown/2022/09/16/XopC59keJiMWjmd.png)
...
![image-20220916221711430](https://oss.itbaima.cn/internal/markdown/2022/09/16/8Mzo36BbYVuRgm9.png)

但是如果直接写上文字的话，会导致编译不通过，因为这段文字也会被认为是程序的一部分。

这种情况，我们就可以告诉编译器，这段文字是我们做的笔记，并不是程序的一部分，那么要怎么告诉编译器这不是代码呢？很简单，我们只需要在前面加上双斜杠就可以了：

![image-20220916222035778](https://oss.itbaima.cn/internal/markdown/2022/09/16/N4rZHt6onGfXuhg.png)

添加双斜杠之后（自动变成了灰色），后续的文本内容只要没有发生换行，那么都会被认为是一段注释，并不属于程序，在编译时会被直接忽略，之后这段注释也不会存在于程序中。但是一旦发生换行那就不行了：

![image-20220916222225047](https://oss.itbaima.cn/internal/markdown/2022/09/16/GiUMCmXewanWJSN.png)
...
![image-20220916222636943](https://oss.itbaima.cn/internal/markdown/2022/09/16/sFhkS2ezONjZvMK.png)
...
![image-20230301113525814](https://oss.itbaima.cn/internal/markdown/2023/03/01/I6nCh49qzyvoZBm.png)

当然各位小伙伴没必要刻意去进行记忆，我们会在学习的过程中逐步认识到这些关键字。新手要辨别一个单词是否为关键字，只需要通过IDEA的高亮颜色进行区分即可，比如：

![image-20220916224129597](https://oss.itbaima.cn/internal/markdown/2022/09/16/qtsjIhSGQoxBYVM.png)
...
![image-20220916225037221](https://oss.itbaima.cn/internal/markdown/2022/09/16/3nUAHINdXMmlxvJ.png)
...
![image-20220916225429474](https://oss.itbaima.cn/internal/markdown/2022/09/16/kT46yi8KNOLWlp3.png)
...
![image-20220916231650085](https://oss.itbaima.cn/internal/markdown/2022/09/16/NLZlDgxz3ci5Idr.png)
...
![image-20220916232420547](https://oss.itbaima.cn/internal/markdown/2022/09/16/76GgjWYz4DPBy1p.png)
...
![image-20220916234540720](https://oss.itbaima.cn/internal/markdown/2022/09/16/YztefPIvLE6y94u.png)
...
![image-20220917105141288](https://oss.itbaima.cn/internal/markdown/2022/09/17/x7bOzyIacpDowKk.png)
...
![image-20220917105849862](https://oss.itbaima.cn/internal/markdown/2022/09/17/1JqHY2so6Qwz4WX.png)
...
![image-20220917110854266](https://oss.itbaima.cn/internal/markdown/2022/09/17/dvizHYa2fCOKhA3.png)
...
![image-20220917111838629](https://oss.itbaima.cn/internal/markdown/2022/09/17/ZgzMUafmYAKoOXt.png)

Java程序在编译为`.class`文件之后，会采用UTF-8的编码格式，支持的字符也非常多，所以你甚至可以直接把变量名写成中文，依然可以编译通过：

![image-20220917112033102](https://oss.itbaima.cn/internal/markdown/2022/09/17/vAnPCgx5ThEUBHe.png)

介绍完了字符之后，我们接着来看看字符串，其实字符串我们在一开始就已经接触到了。字符虽然可以表示一个中文，但是它没办法表示多个字符：

![image-20220917114628564](https://oss.itbaima.cn/internal/markdown/2022/09/17/JmdQkSw2qc4ZTuW.png)
...
![image-20220917115424504](https://oss.itbaima.cn/internal/markdown/2022/09/17/1TtJdKcvRWPfAI2.png)
...
![image-20220917141359260](https://oss.itbaima.cn/internal/markdown/2022/09/17/KovME45pl2sPiBN.png)

小数和整数一起计算同样会发生隐式类型转换：

![image-20220917141955891](https://oss.itbaima.cn/internal/markdown/2022/09/17/jxW3KfwBACidyMY.png)
...
![image-20220917145901135](https://oss.itbaima.cn/internal/markdown/2022/09/17/TeUCBM9ZzINuoa8.png)
...
![image-20220917150010919](https://oss.itbaima.cn/internal/markdown/2022/09/17/URJxsgXvzYMQh8t.png)
...
![image-20220917141816599](https://oss.itbaima.cn/internal/markdown/2022/09/17/TdhHPN64UnyFozq.png)
...
![image-20220917142201392](https://oss.itbaima.cn/internal/markdown/2022/09/17/3zjJoeL6bgdRqNA.png)
...
![image-20220917150256987](https://oss.itbaima.cn/internal/markdown/2022/09/17/En2uzTl5PFgKeNX.png)
...
![image-20220917151028191](https://oss.itbaima.cn/internal/markdown/2022/09/17/Kt6rfkYE1HSvNnl.png)
...
![image-20220917223047110](https://oss.itbaima.cn/internal/markdown/2022/09/17/Z1yAPOKe8IVvFUt.png)
...
![image-20220917224320699](https://oss.itbaima.cn/internal/markdown/2022/09/17/tJQxnace7y4VdlY.png)
...
![image-20220917231014796](https://oss.itbaima.cn/internal/markdown/2022/09/17/DdvU3aQmE25KbxM.png)
...
![image-20220918112006020](https://oss.itbaima.cn/internal/markdown/2022/09/18/A8lRmNZCqxLStwQ.png)

调试模式在我们后面的学习中非常重要，影响深远，所以说各位小伙伴一定要学会。调试也很简单，我们只需要点击右上角的调试选项即可（图标像一个小虫子一样，因为调试的英文名称是Debug）

![image-20220918112101677](https://oss.itbaima.cn/internal/markdown/2022/09/18/VKMGoJazvXAnh2k.png)

调试开始时，我们可以看到程序在断点位置暂停了：

![image-20220918112227207](https://oss.itbaima.cn/internal/markdown/2022/09/18/Cdq1ifFvHwMuO29.png)

此时我们可以观察到当前的局部变量`i`的值，也可以直接在下方的调试窗口中查看：

![image-20220918112409944](https://oss.itbaima.cn/internal/markdown/2022/09/18/e6AODRMCgqmGwTy.png)

随着循环的进行，i的值也会逐渐自增：

![image-20220918112628585](https://oss.itbaima.cn/internal/markdown/2022/09/18/bS1DxpgwOfWhujy.png)
...
![image-20220918112923978](https://oss.itbaima.cn/internal/markdown/2022/09/18/2aO9Ro5yfMUvhNc.png)
...
![img](https://oss.itbaima.cn/internal/markdown/2022/09/18/zy1wuvj6gfHmAZS.jpg)

现在我们要做的是在我们的程序中，也打印出这样的一个乘法表出来，请你设计一个Java程序来实现它。

![img](https://oss.itbaima.cn/internal/markdown/2022/09/18/Iek7OnbRoTw46Cl.jpg)
